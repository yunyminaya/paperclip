...
`POST /api/companies/:companyId/operating-loop/run` creates one idempotent daily operating-review issue for the enabled CEO agent and queues its assignment when the heartbeat scheduler is available. Board operators and that company's CEO can trigger the run; other agents cannot. The issue asks the CEO to identify the highest-return action, choose continue/change/stop, delegate governed child work, require independent verification, and report expected return and cost. Existing approval, budget, tool-access, and audit controls still apply.
...
`GET /api/companies/:companyId/operating-loop/capabilities` combines the existing governed MCP catalog with company skills. It reports readiness for sales/CRM, banking/cash, accounting/tax, communications, and analytics from the active CEO's effective access profile. A domain is `ready` only when Paperclip sees both an installed, enabled, active, healthy MCP tool allowed for that CEO and a compatible skill. The daily review embeds missing capability information and directs the CEO to create setup work instead of attempting an unavailable action. Tool profiles, approvals, credential isolation, and invocation policies remain authoritative at execution time.

## Known limits

- The daily run endpoint can immediately queue the CEO assignment, but this slice does not silently schedule recurring runs for existing companies.
